由于译者时间及水平有限，翻译难免有误，欢迎批评指正！若发现有错误的地方可以在github repo中提交PR，感谢大家的支持！欢迎各路大神帮忙改进和review，Github地址：https://github.com/PyCN/PTR
...
### Changes in the Python API
  ...
  * [`smtpd.SMTPChannel`][308]和[`smtpd.SMTPServer`][309]的构造函数的_decode\_data_参数现在默认是`False`。这意味着参数将以关键字的形式传递给[`process_message()`][310]，默认是一个bytes对象。如果代码已经在3.5中根据弃用告警做出了相应的调整则将不会被影响。

  * [`json`][317]模块中的函数[`dump()`][311], [`dumps()`][312], [`load()`][313]和[`loads()`][314]，类[`JSONEncoder`][315]和[`JSONDecoder`][316]的构造函数的所有的可选参数现在都只能是关键字参数。(Contributed by Serhiy Storchaka in [issue 18726][319])

  * 如果[`type`][320]的子类没有重载`type.__new__`，那么将不能用单个参数的形式来获得一个对象的类型。

  * 作为[**PEP 487**][321]的一部分，传递给[`type`][322]的关键字参数（除了`metaclass`外）现在将会原样传递给[`object.__init_subclass__()`][323]。这意味着`type.__new__()`和`type.__init__()`现在都可以接受任意的关键字参数，但[`object.__init_subclass__()`][324] (在`type.__new__()`中调用)默认不接受。自定义的元类如果接受额外的关键字参数的话现在需要调整他们调用`type.__new__()`的方式（直接或间接地通过[`super`][325]）。

  * 移除了`distutils.command.sdist.sdist`的`default_format`属性。以后在所有的平台上都将使用gzipped tarfile格式，并且不支持选择任何平台特有的格式。如果在有的环境中需要在Windows上构建发行版并且要求zip格式，可以采用一个含有如下内容的`setup.cfg`配置文件：
```     [sdist]
...
```
  ...
  * [`DictReader`][329]现在返回的行是[`OrderedDict`][330]类型。(Contributed by Steve Holden in [issue 27842][331].)

  * 在不支持的平台上，[`crypt.METHOD_CRYPT`][332]将不会被添加到`crypt.methods`中。(Contributed by Victor Stinner in [issue 25287][333].)

  * [`namedtuple()`][334]的_verbose_和_rename_参数现在只支持关键字形式。 (Contributed by Raymond Hettinger in [issue 25628][335].)

  * 在Linux上，[`ctypes.util.find_library()`][336]现在会搜索`LD_LIBRARY_PATH`来获得共享库。 (Contributed by Vinay Sajip in [issue 9998][337].)

  * 类[`imaplib.IMAP4`][338]现在会处理由服务器发来的消息中包含`']'`字符的标识来增强实用性。(Contributed by Lita Cho in [issue 21815][339].)

  * 函数`mmap.write()`现在会像其他写操作一样返回已写的字节数。 (Contributed by Jakub Stasiak in [issue 26335][340].)

  * 函数[`pkgutil.iter_modules()`][341]和[`pkgutil.walk_packages()`][342]现在会返回一个[`ModuleInfo`][343]（namedtuple)。(Contributed by Ramchandra Apte in [issue 17211][344].)

  * 对于[`re.sub()`][345]，现在如果替换模板中有无效的组下标，即使这个模板没有匹配到字符，也会抛出一个异常。异常信息中会包含这个组下标以及它在模板中的位置。(Contributed by SilentGhost, Serhiy Storchaka in [issue 25953][346].)

  * [`zipfile.ZipFile`][347]现在对于不识别的压缩值会抛出[`NotImplementedError`][348]而不是[`RuntimeError`][349]。另外，对一个已关闭的ZipFile调用[`ZipFile`][350]的方法，或者对一个只读的ZipFile调用[`write()`][351]方法会抛出[`ValueError`][352]而不是[`RuntimeError`][353]。

  * 如果在使用自定义元类的同时还使用了零参数的[`super()`][354]，或者在方法中直接引用闭包中隐含的`__class__`，那隐含的`__classcell__`必须被传递给`type.__new__`。如果没有这么做那么在3.6中会抛出一个[`DeprecationWarning`][355]，在以后的版本中会抛出[`RuntimeWarning`][356]。
